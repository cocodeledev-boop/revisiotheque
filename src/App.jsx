...
export default function App() {
  ...
  // Charger les fiches depuis Firebase
  ...
  const handleSubmit = async () => {
    ...
    try {
      ...
      setFiches((prev) => [{ id: docRef.id, ...form, createdAt: { seconds: Date.now() / 1000 } }, ...prev]);
      ...
      alert("Erreur lors de la publication. Vérifie ta connexion.");
    } finally {
      setSaving(false);
    }
  };
  ...
  return (
    <div style={s.root}>
        ...
        <div style={s.overlay} onClick={(e) => e.target === e.currentTarget && setShowForm(false)}>
          <div style={s.modal}>
            ...
            <select style={s.input} value={form.matiere}
              ...
              {SUBJECTS.map((s) => <option key={s.label}>{s.label}</option>)}
            </select>
            ...
            <div style={s.modalFooter}>
              ...
              <button
                style={{ ...s.submitBtn, opacity: (!form.auteur || !form.titre || !form.contenu || saving) ? 0.5 : 1 }}
                onClick={handleSubmit}
                disabled={!form.auteur || !form.titre || !form.contenu || saving}>
      ...
      <footer style={s.footer}>
        📚 RévisoThèque — Les fiches sont partagées et conservées pour toujours · Propulsé par Firebase & Vercel
      </footer>
    </div>
  );
}
...
const s = {
  ...
  submitBtn:   { background: "#1A1A1A", color: "#FFF", border: "none", borderRadius: 8, padding: "10px 22px", cursor: "pointer", fontWeight: 700, fontFamily: "Georgia, serif", fontSize: 14 },
